#!/usr/bin/env bash
# FullInitialization performs the complete container initialization in a single script.
#
...
set -Eeuo pipefail
trap 'rc=$?; echo -e "\n!! FullInitialization failed at line $LINENO while running: $BASH_COMMAND (exit $rc)" >&2; exit $rc' ERR
